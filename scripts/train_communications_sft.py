"""Train the communications agent SFT model with LoRA."""
...
OUTPUT_DIR = "checkpoints/communications_sft_final/"
TRAIN_FILE = "data/communications_sft_train.jsonl"
LOSS_LOG = "logs/communications_sft_loss.txt"


def build_lora_config() -> Any:
    """Build LoRA configuration for communications SFT."""
    try:
...
def build_sft_config() -> Any:
    """Build SFT trainer config (4-bit friendly — no fp16 in SFTConfig)."""
    try:
...
def main() -> None:
    """Execute communications SFT training."""
    final_loss = train()
    print(f"Communications SFT training complete. Loss: {final_loss}")
